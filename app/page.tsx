  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_50%_-20%,#134e4a_0%,#070b14_45%)]"><Navbar />
    <main>
      <section className="mx-auto max-w-7xl px-5 py-24 text-center sm:py-32 lg:px-8">
        ...
        <p className="mt-5 text-xs font-medium uppercase tracking-[0.16em] text-slate-600">For applied AI engineers · agent teams · technical release reviewers</p>
        <div className="mx-auto mt-14 max-w-4xl rounded-2xl border border-slate-700 bg-slate-950/80 p-4 text-left shadow-2xl shadow-teal-950/20 sm:p-6">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-4"><div><p className="text-xs font-medium text-slate-500">Candidate release</p><p className="mt-1 font-semibold text-white">Support Agent Prompt · v2.1</p></div><span className="rounded-full border border-amber-400/30 bg-amber-400/10 px-3 py-1.5 text-xs font-semibold text-amber-200">Needs Review</span></div>
          <div className="grid gap-3 pt-4 sm:grid-cols-3">{[["Deterministic score", "82.4"], ["Tests passed", "4 of 5"], ["Safety failures", "0"]].map(([label, value]) => <div key={label} className="rounded-xl bg-slate-900/80 p-4"><p className="text-xs text-slate-500">{label}</p><p className="mt-2 text-xl font-bold text-white">{value}</p></div>)}</div>
          <p className="mt-4 text-xs leading-5 text-slate-500">A clear recommendation backed by saved, per-test evidence—not an opaque model judgment.</p>
        </div>
